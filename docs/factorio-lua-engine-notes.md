...
- **观察者效应(真实案例)**:E2E 台架里 `on_nth_tick(120)` 的采样函数用
  `find_entities_filtered` 做全表面查询,单次 7~12ms 记入 scriptUpdate,
  被当成 mod 的"周期性尖峰"追查了一整轮——最差 tick 全是 120 的整数倍
  才暴露。教训:①监控代码自己要零成本(build 时把实体引用存进 storage,
  采样直接读);②看到可疑落点先对 **自己仪器的节拍** 取模;③
  `find_entities_filtered` 全表面查询是毫秒级操作,不属于任何热路径。
